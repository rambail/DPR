...
class EnergyRequirement:
    def __init__(self, power_params, working_hours, yearly_headways,
                    section_length, train_weight_aw4,years):
        """
        ...
        self.train_weight_aw4 = train_weight_aw4
        self.years = years
    ...
    def compute_total_energy(self, traction_mw, auxiliary_mw, years):
        ...
        for year in years:
            ...
            tr_units = (traction_mw[year]
                        * self.working_hours['Hours']
                        * self.working_hours['Days']
                        / 1000
                        )
            aux_units = (auxiliary_mw[year]
                        * self.working_hours['Hours']
                        * self.working_hours['Days']
    ...
    def compute_dpr_data(self):
        traction_energy_raw, traction_energy_regen_eff = self.compute_traction_energy(
                                                                self.yearly_headways,
                                                                self.section_length,
                                                                self.train_weight_aw4
                                                            )
        aux_energy_raw, aux_energy_eff = self.compute_auxiliary_energy(self.years)
        total_units, max_demand = (self.compute_total_energy(traction_energy_regen_eff,
                                                                aux_energy_eff, self.years))
        return [traction_energy_raw,
                traction_energy_regen_eff,
                aux_energy_raw,
                aux_energy_eff,
                total_units,
                max_demand
